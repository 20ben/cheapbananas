...
class LettaManager:
    ...
    def __init__(self):
        ...
        # Load all minion IDs (adjust range as needed)
        self.minion_ids = []
        for i in range(1, 9):  # Checking for up to 10 minions
            minion_id = os.environ.get(f"MINION_ID{i}")
            if minion_id:
                self.minion_ids.append(minion_id)
            else:
                break  # Stop when we don't find the next minion
        
        if len(self.minion_ids) == 0:
            raise ValueError("No MINION_ID environment variables found")
        
        logger.info(f"Initialized LettaManager with {len(self.minion_ids)} minions")
